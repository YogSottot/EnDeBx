...
class InstallBitrixCommand extends Command
{
    ...
    private function createEventSubscriber(SymfonyStyle $ss): EventSubscriberInterface
    {
        return new class($ss) implements EventSubscriberInterface {
            ...
            {
                $this->ss = $ss;
                $this->debugSteps = filter_var(
                    getenv('BITRIX_INSTALL_DEBUG_STEPS') ?: '0',
            ...
            public function __invoke(object $event)
            {
                if ($event instanceof BitrixDistributiveReady) {
                    ...
                    if ($this->debugSteps) {
                        $this->ss->note(sprintf(
                            'Пошаговый debug включён%s',
                            getenv('BITRIX_INSTALL_DEBUG_DIR')
                                ? ', каталог: ' . getenv('BITRIX_INSTALL_DEBUG_DIR')
                                : ''
                        ));
                    }
                    ...
                    if (($debugDir = getenv('BITRIX_INSTALL_DEBUG_DIR')) !== false && $debugDir !== '') {
                        $this->ss->note('debug_dir = ' . $debugDir);
                    }
