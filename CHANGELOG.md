...
- Отключение `xdebug` для обычных PHP SAPI переведено с удаления `conf.d` symlink на `phpdismod`, чтобы обновления пакета не восстанавливали модуль; отдельный `php-fpm-xdebug` продолжает использовать установленный `php-xdebug`.
