...
- Добавлена переменная `BX_DOWNLOAD_BITRIXSETUP` для управления загрузкой `bitrixsetup.php` и `restore.php` при создании `full`-сайтов и при первичной установке окружения.
...
- Для новых `full`-сайтов и для сайта `default` при первичной установке загрузка `bitrixsetup.php` и `restore.php` теперь по умолчанию отключена; вместо этого создаётся шаблон `index.php`. Для явного включения загрузки добавлена настройка `BX_DOWNLOAD_BITRIXSETUP=Y`.
...
- Для сценариев создания `full`-сайта и первичной установки снижён риск CVE-2022-29268: автозагрузка `bitrixsetup.php` и `restore.php` больше не выполняется без явного включения.
