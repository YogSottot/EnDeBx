...
1) Скрипт будет работать, даже если закрыть окно браузера, пока все не выполнит. За процессом можно наблюдать через `mysqladmin -i 1 processlist | grep -i ALTER`
